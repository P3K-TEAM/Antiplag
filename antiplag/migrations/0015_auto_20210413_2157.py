...
def migrate_result_to_new_model(apps, schema_editor):
    ...
    def parse_matched_doc(doc):
        ...
        # newer documents have id in matched_docs
        if "id" in doc:
            try:
                # if is valid UUID, it's internal document
                result["match_id"] = str(uuid.UUID(doc["id"], version=5))
        ...
        # if id is missing
            ...
            # string in database contains type identifier from Document.__str__ method
            result["match_name"] = re.sub(r" \((FILE|TEXT)\)$", "", doc["name"])

            # check existing document by name
            document = (
                Document.objects.filter(name=result["match_name"])
                .order_by("id")
                .first()
            )
            if document:
                result["match_id"] = document.id
                result["match_type"] = MatchType.UPLOADED
            else:
                # if matched_doc name is not an internal document, check for name in elastic
                document = list(
                    Search(index="documents").query(
                        MatchPhrase(name=result["match_name"])
                    )
                )[0]
                result["match_id"] = document.meta["id"]
                result["match_type"] = MatchType.CORPUS
